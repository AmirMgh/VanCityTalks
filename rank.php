...
class Rank{
	...
	function getPrpAvg($prpRecords) {
		$sum_prptax = 0;
		foreach ($prpRecords as $record) {
			$sum_prptax += $record['VALUE'];
		}
		return $sum_prptax/$prpRecords->count();
	}
	...
	function computeScores($testData) {
		try {	
			...
			foreach ($testData as $td) {
				$res = array();
				//var_dump($this->get_query($td['lat'], $td['lon'], $td['rad']));
				...
				$qprptx = $this->getPrpAvg($this->col_prptx->find($this->get_query($td['lat'], $td['lon'], $td['radius'])));
